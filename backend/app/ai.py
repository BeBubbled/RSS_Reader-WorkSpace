...
from app.models import AIArtifact, AIFunction, AIJob, AIModel, AIProvider, Entry, Notification, RoutingPolicy, Workflow
...
async def queue_entry_job(session:AsyncSession,entry:Entry,function_key:str,language:str|None=None)->AIJob:
 ...
 job=AIJob(function_id=function.id,status="queued",entry_set_hash=hashlib.sha256(str(entry.id).encode()).hexdigest()); session.add(job); await session.commit(); await session.refresh(job); return job
